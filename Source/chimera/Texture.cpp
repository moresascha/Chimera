...
ID3D11ShaderResourceView* Texture::GetShaderResourceView(VOID) 
{
    return this->m_pTextureView;
}
...
BOOL Texture2D::VCreate(VOID) 
{
    ...
    if(m_texDesc.MiscFlags & D3D11_RESOURCE_MISC_GENERATE_MIPS)
    {
        ...
        if(FAILED(d3d::GetDevice()->CreateTexture2D(&m_texDesc, NULL, &m_pTexture))) return FALSE;
        D3D11_BOX destBox;
        destBox.left = 0;
        ...
        destBox.front = 0;
        destBox.back = 1;
        d3d::GetContext()->UpdateSubresource(m_pTexture, 0, &destBox, m_rawData, m_texDesc.Width * 4, 0);
    }
    else
    {
        ...
        //LOG_ERROR("asdasd");
        if(FAILED(d3d::GetDevice()->CreateTexture2D(&m_texDesc, pSubData, &m_pTexture))) return FALSE;
    }
    ...
    */
    if(GetBindflags() & D3D11_BIND_SHADER_RESOURCE)
    {
        ...
        if(FAILED(d3d::GetDevice()->CreateShaderResourceView(m_pTexture, &viewDesc, &m_pTextureView))) return FALSE;

        if(m_texDesc.MiscFlags & D3D11_RESOURCE_MISC_GENERATE_MIPS)
        {
            d3d::GetContext()->GenerateMips(m_pTextureView);
        }
    }
