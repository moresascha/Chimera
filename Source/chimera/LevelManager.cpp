...
namespace tbd
{
    ...
    {
        m_formatSaver[0] = new tbd::SaveXMLLevel();
    }
    ...
    BOOL SaveXMLLevel::VSaveLevel(ILevel* level, LPCSTR file)
    {
        ...
        std::map<ActorId, std::shared_ptr<tbd::Actor>>& actors = level->VGetActors();
        ...
        for(auto it = actors.begin(); it != actors.end(); ++it)
        {
            ...
            if(!actor->HasComponent<tbd::ParentComponent>(tbd::ParentComponent::COMPONENT_ID))
            {
                ...
                std::shared_ptr<ISceneNode> node = app::g_pApp->GetHumanView()->GetSceneGraph()->FindActorNode(actor->GetId());
                ...
                {                    
                    ...
                    std::shared_ptr<tbd::Actor> child = app::g_pApp->GetLogic()->VFindActor(it3->get()->VGetActorId());
