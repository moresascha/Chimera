...
__global__ void reorderEdges(Edge dst, Edge src, uint N)
{
    ...
    ONE_TO_ONE_CPY(type);
    dst.indexedEdge[id] = src.indexedEdge[id];
}
...
__global__ void reorderSplits(Split dst, Split src, uint N)
{
    ...
    ONE_TO_ONE_CPY(axis);
    ONE_TO_ONE_CPY(v);
    dst.indexedSplit[id] = src.indexedSplit[id];
}
...
float __device__ getSAH(BBox* node, int axis, float split, int primBelow, int primAbove, float traversalCost = 0.25f, float isectCost = 1)
{
    float cost = FLT_MAX;
        ...
        uint N) 
{
    ...
    int edgesInNode = 2 * nodes.contentCount[nodeIndex];
...
__global__ void createNodes(
        uint* scannedEdgeMask,
        uint* edgeMask,
        Split splits, 
        Node nodes, 
        uint depth,
        bool forceLeaf)
{
    ...
    uint edgesBeforeMe = 0;

    for(int j = 0; j < id; ++j)
    {
        edgesBeforeMe += 2 * nodes.contentCount[offset + j]; //this is awful, fix it, prefix sum over nodescontent per node please
    }
    ...
    int below = (int)splits.below[edgesBeforeMe];
    int above = (int)splits.above[edgesBeforeMe];
    ...
    float s = splits.v[edgesBeforeMe];
    byte axis = splits.axis[edgesBeforeMe];

    nodes.splitAxis[parentIndex] = axis;
    ...
    int os = max(0, (int)scannedEdgeMask[edgesBeforeMe + max(0, below)]);
    uint elemsleft = max(0, (int)scannedEdgeMask[edgesBeforeMe + max(0, below)] - (int)scannedEdgeMask[edgesBeforeMe]);
    uint elemsright = max(0, (int)scannedEdgeMask[edgesBeforeMe + above + below - 1] - os) + (int)edgeMask[edgesBeforeMe + max(0, below + above - 1)];

    nodes.isLeaf[leftChildIndex] = forceLeaf;
    nodes.isLeaf[rightChildIndex] = forceLeaf;
